...
describe('ParticleSphereMenu',()=>{
  ...
  it('mantiene i beacon posteriori visibili ma non interattivi',()=>{render(<ParticleSphereMenu items={items}/>);const beacon=screen.getByRole('link',{name:/Lab/});beacon.dataset.occluded='true';beacon.style.opacity='.14';beacon.style.pointerEvents='none';beacon.tabIndex=-1;expect(beacon).toHaveStyle({opacity:'.14',pointerEvents:'none'});expect(beacon).toHaveAttribute('tabindex','-1')});
});

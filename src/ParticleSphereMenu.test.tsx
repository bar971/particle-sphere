...
const items:SphereMenuItem[]=[{id:'lab',title:'Lab',summary:'Esperimenti',href:'/lab',icon:'◌',position:{latitudeDeg:0,longitudeDeg:0}},{id:'note',title:'Note',summary:'Appunti',href:'/note',position:{latitudeDeg:0,longitudeDeg:60}}];
describe('ParticleSphereMenu',()=>{
  let frame:FrameRequestCallback|undefined;
  beforeEach(()=>{frame=undefined;vi.stubGlobal('requestAnimationFrame',vi.fn(callback=>{frame=callback;return 1}));vi.stubGlobal('cancelAnimationFrame',vi.fn())});
  ...
  it('non renderizza il dock mobile',()=>{render(<ParticleSphereMenu items={items}/>);expect(screen.queryByRole('button',{name:'Apri Lab'})).not.toBeInTheDocument()});
  it('inoltra la navigazione dalla CTA',()=>{const onNavigate=vi.fn((_item,event)=>event.preventDefault());render(<ParticleSphereMenu items={items} onNavigate={onNavigate}/>);fireEvent.click(screen.getByRole('navigation',{name:'Navigazione principale'}).querySelector<HTMLAnchorElement>('.beacon')!);fireEvent.click(screen.getByRole('link',{name:/Esplora/i}));expect(onNavigate).toHaveBeenCalledWith(items[0],expect.anything())});
  ...
  it('mostra una sola etichetta contestuale e privilegia il beacon selezionato visibile',()=>{render(<ParticleSphereMenu items={items}/>);frame?.(16);const navigation=screen.getByRole('navigation',{name:'Navigazione principale'});expect(navigation.querySelectorAll('[data-label-visible="true"]')).toHaveLength(1);fireEvent.click(navigation.querySelectorAll<HTMLAnchorElement>('.beacon')[1]);frame?.(32);expect(navigation.querySelector('[data-label-visible="true"]')).toHaveAttribute('href','/note')});
});
